...
pro erg_load_orb, $
  ...
  if keyword_set(pre) then begin 
     ...
     remotedir = 'http://'+uname+':'+passwd  $ 
               + '@ergsc.isee.nagoya-u.ac.jp/data/ergsc/satellite/erg/orb_pre/' 
     ;help, parse_url(remotedir) 
     ; 
     ;localdir =    !erg.local_data_dir      + 'satellite/erg/orb/' 
     localdir = root_data_dir() + 'ergsc/satellite/erg/orb/' 
     ;  
     ;Relative file path 
     ...
     options, prefix+'pos_'+['gse','gsm','sm','lmlatmlt'], 'colors', [2,4,6]
     options, prefix+'pos_'+'lmlatmlt', 'labels', ['Req','MLAT','MLT']
     options, prefix+'pos_'+'eq', 'labels', ['Req','MLT']
     options, prefix+'pos_iono_'+['north','south'], 'labels', ['GLAT','GLON']
     options, prefix+'pos_blocal', 'labels', 'B(model)!C_at_ERG'
     options, prefix+'pos_beq', 'labels', 'B(model)!C_at_equator'
     options, prefix+'pos_b'+['local','eq'], 'ylog', 1
